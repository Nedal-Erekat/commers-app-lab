...
Real Azure, minimal footprint: a single small AKS node pool (control plane is free), Basic-tier SQL/Redis, everything defined in Bicep so `az deployment` up/down is one command. Spin up before a demo, tear down after — the teardown script ships alongside the IaC in milestone 5.
