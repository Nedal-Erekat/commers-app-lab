...
| `catalog.yaml`, `identity.yaml`, `cart.yaml`, `order.yaml`, `assistant.yaml` | Deployment + ClusterIP Service, each with `/health` liveness/readiness probes |
...
`assistant.yaml` reads `Anthropic__ApiKey` from `app-secrets` — `deploy.sh` requires `ANTHROPIC_API_KEY` to be set before it will create that secret.

All services request modest resources (50m CPU / 64–96Mi memory) to fit eight services plus AKS system pods on a single `Standard_B2s` node — this is a demo-sized deployment, not a production capacity plan.
